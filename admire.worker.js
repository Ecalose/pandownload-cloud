const BDUSS = 'k5vM1phbnU3aEZsek5BeVJHY0JGZjVJdHUtflNTZWNoRWQ5ZGFpZks2ellzUHRmSUFBQUFBJCQAAAAAAAAAAAEAAABfUiyhenFmbzYyAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAANgj1F~YI9RfQ' //**INPUT YOUR BDUSS HERE**
const STOKEN = '2653e273f69b26f6317ad64ab5ad015e65af47099c39c7e47a8e31933e209bb9' //**INPUT YOUR STOKEN HERE**
const SVIPBDUSS = 'k5vM1phbnU3aEZsek5BeVJHY0JGZjVJdHUtflNTZWNoRWQ5ZGFpZks2ellzUHRmSUFBQUFBJCQAAAAAAAAAAAEAAABfUiyhenFmbzYyAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAANgj1F~YI9RfQ' //**INPUT YOUR SVIP BDUSS HERE**
const SVIPSTOKEN = '2653e273f69b26f6317ad64ab5ad015e65af47099c39c7e47a8e31933e209bb9' //**INPUT YOUR SVIP STOKEN HERE** (optional, rapid need) 
const INDEX_URL = 'index.php' // Input your index url here
const AUTH_USER = '' //**INPUT BASIC AUTH USERNAME (optional)**
const AUTH_PASS = '' //**INPUT BASIC AUTH SUPER SECRET PASSWORD (optional)**
const rand = '&devuid=O|50604DF4593E4C69A4092B73AB5D324A&rand=d2b6349f0ff36127bcf43ec4681de82f838b1ff7&time=1609854290'
...
<li class="nav-item"><a class="nav-link" href="https://t.me/fixpds" target="_blank">Telegram</a></li>
</ul>
</div>
</div>
</nav>
...
<li>填入： netdisk;P2SP;2.2.60.26</li>
<li>切换到浏览器，长按“下载链接”，选择复制链接地址</li>
<li>在ADM中添加任务并开始</li>
</ol>
<h4>IDM</h4>
<ol>
<li>选项 -> 下载 -> 手动添加任务时使用的用户代理（UA）-> 填入 <b>netdisk;P2SP;2.2.60.26</b></li>
...
<li>New User-Agent String 填入 netdisk;P2SP;2.2.60.26</li>
<li>Group 填入 百度网盘</li>
...
<li>添加自定义UA：netdisk;P2SP;2.2.60.26</li>
</ol>
<h4>Alook浏览器（IOS）</h4>
<ol>
<li>设置 -&gt; 通用设置 -&gt; 浏览器标识 -&gt; 移动版浏览器标识 -&gt; 自定义 -><br> 填入 <b>netdisk;P2SP;2.2.60.26</b></li>
</ol>
</section>
</div>
</p>
...
const headerOption = ['User-Agent: netdisk;P2SP;2.2.60.26']
let post
let postVer
if(token != ""){
postVer = JSON.stringify({	
...
<li class="nav-item">
<a class="nav-link" href="https://github.com/TkzcM/baiduwp" target="_blank">GitHub</a></li>
<li class="nav-item"><a class="nav-link" href="https://t.me/fixpds" target="_blank">Telegram</a></li>
</ul>
</div>
</div>
...
function getdi(rl){
	...
	//const trueLink = 'qdall01.baidupcs.com' + dldata['path']
	return rl
	var mesg="1"
	var getTrueLink = fetch('https://'+rl,{
          headers:{
            'user-agent': 'netdisk;P2SP;2.2.60.26'
          },
          redirect:"manual" 
	...
	for (i=0;i<10;i++){
	if(getTrueLink.status == 302){
		return getTrueLink.headers.get('Location').substring(7)
	}
	getTrueLink = fetch('https://'+rl,{
          headers:{
            'user-agent': 'netdisk;P2SP;2.2.60.26'
          },
          redirect:"manual" 
...
const dlRapid = async request => {
  ...
  if(fjson['errno'] === 0){
    ...
    const getRealLink = await fetch('http://d.pcs.baidu.com/rest/2.0/pcs/file?app_id=250528&method=locatedownload&check_blue=1&es=1&esl=1&origin=dlna&path='+encodeURIComponent(path)+'&dtype=1&err_ver=1.0&ehps=0&open_pflag=0&clienttype=8&channel=00000000000000000000000000000000' + rand ,{
      headers:{
        'user-agent': 'netdisk;P2SP;2.2.60.26',
        'Cookie': 'BDUSS=' + SVIPBDUSS + ';'
      },
    ...
    if(getRealLink.status == 200){
	  ...
	  const realLink = dldata.server[0] + dldata['path']
        /*const getTrueLink = await fetch('http://'+realLink,{
          headers:{
            'user-agent': 'netdisk;P2SP;2.2.60.26'
          },
        ...
        //const trueLink = getTrueLink.headers.get('Location').substring(7)
	//const trueLink = 'qdall01.baidupcs.com' + dldata['path']
	// <a >`+ getRealLink.text +`</a>
		const trueLink = getdi(realLink)
		if(trueLink !="1"){
...
const download = async request => {
  ...
  var createFolderForm = new FormData()
  createFolderForm.append('path','/baiduwp/'+timestamp)
  createFolderForm.append('isdir','1')
  const createFolder = await fetch('http://pan.baidu.com/api/create?a=commit&channel=chunlei&web=1&app_id=250528&bdstoken='+bdstoken+'&clienttype=0',
  {
    headers:{
      'Referer':'http://pan.baidu.com/s/',
      'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.514.1919.810 Safari/537.36',
      'Cookie':'BDUSS=' + SVIPBDUSS + '; '
      +  'STOKEN=' + SVIPSTOKEN + ';'
    },
  ...
  saveForm.append('fsidlist','['+fs_id+']')
  saveForm.append('path','/baiduwp/'+timestamp)
  const saveFile = await fetch('http://pan.baidu.com/share/transfer?shareid='+share_id+'&from='+uk+'&channel=chunlei&web=1&app_id=250528&bdstoken='+bdstoken+'&clienttype=0',
    {
      headers:{
        'Referer':'http://pan.baidu.com/s/',
        'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.514.1919.810 Safari/537.36',
  ...
  try{
    const saveResult = JSON.parse(await saveFile.text())
    if(saveResult['errno'] == 0 || saveResult['info'][0].errno == -30){
      let path
      if(saveResult['errno'] == 12){
        path = '/baiduwp/'+timestamp+saveResult['info'][0]['path']
      }
      else{path = saveResult['extra']['list'][0]['to']}
      const getRealLink = await fetch('http://d.pcs.baidu.com/rest/2.0/pcs/file?app_id=250528&method=locatedownload&check_blue=1&es=1&esl=1&origin=dlna&path='+encodeURIComponent(path)+'&dtype=1&err_ver=1.0&ehps=0&open_pflag=0&clienttype=8&channel=00000000000000000000000000000000' + rand,{
        headers:{
          'user-agent': 'netdisk;P2SP;2.2.60.26',
          'Cookie': 'BDUSS=' + SVIPBDUSS + ';'
