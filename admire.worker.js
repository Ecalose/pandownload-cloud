...
<div class="container">
<a class="navbar-brand" href="https://wx.admire.workers.dev/">
...
<ul class="navbar-nav">
<li class="nav-item">
<a class="nav-link" href="https://wx.admire.workers.dev/">主页</a>
</li>
<li class="nav-item">
<a class="nav-link" href="https://github.com/Admirepowered/pandownload-cloud" target="_blank">Github</a>
</li>
<li class="nav-item">
<a class="nav-link" href="https://github.com/Admirepowered/pandownload-cloud" target="_blank">Github</a>
</li>
<li class="nav-item">
<a class="nav-link" href="https://github.com/Admirepowered/pandownload-cloud">Github</a>
</li>
</ul>
</div>
</div>
...
<script>
 ...
 function dl(fs_id, timestamp, sign, randsk, share_id, uk) {
    ...
    form.append('<input type="hidden" name="surl" value="'+sign+'">');
    form.append('<input type="hidden" name="pwd" value="'+share_id+'">');
    form.append('<input type="hidden" name="path" value="'+uk+'">');
    $(document.body).append(form);
    form.submit();
...
const generate = async request => {
  ...
  var path = text.get('path')
  //filecontent+=surl+","+pwd+","+path+","+encodeURI(path)+","
  ...
  //filecontent+=path
  //return new Response(path, {status: 200})
  async function getFileList(surl_1,pwd,path){
	const filedata = new FormData()
	filedata.append('shorturl',surl_1)
	filedata.append('dir',path)
	filedata.append('root',tt)
	filedata.append('page','1')
	filedata.append('num','100')
	filedata.append('order','time')
	filedata.append('pwd',pwd)
	
    const res2 = await fetch('https://pan.baidu.com/share/wxlist?channel=weixin&version=2.2.2&clienttype=25&web=1',{
      method: 'POST',
	  body: filedata,
      headers: {
  ...
  const filejson = await getFileList(surl,pwd,path)
  //return new Response(filejson, {status: 200} )
  //return new Response(filejson.server_time, {status: 200} )
  var time = filejson.server_time
    for(var i=0;i<filejson.data.list.length;i++){
      const file = filejson.data.list[i]
	  //return new Response(file.size, {status: 200} )
...
<a href="javascript:void(0)" onclick="dl('`+ file.fs_id + `',`+ time +`,'`+ sign +`','` + randsk + `','`+shareid+`','`+ uk +`')">`+file.server_filename+`</a>
<span class="float-right">`+ file.size +`</span>
	  </li>`*/
	  //return new Response(filebody+filecontent+filefoot, {status: 200})
	        filecontent += `<li class="list-group-item border-muted rounded text-muted py-2">
<i class="far fa-file mr-2"></i>
<a href="javascript:void(0)" onclick="dl('`+ file.fs_id + `',`+ file.isdir +`,'`+ surl +`','` + file.dlink + `','`+pwd+`','`+ time +`')">`+file.server_filename+`</a>
<span class="float-right">`+ file.size +`</span>
	  </li>`
		...
		//filecontent+=file.dlink+","
	  
	  }
	  else{
		  filecontent += `<li class="list-group-item border-muted rounded text-muted py-2">
<i class="far fa-file mr-2"></i>
<a href="javascript:void(0)" onclick="dl('`+ file.fs_id + `',`+ file.isdir +`,'`+ surl +`','` + file.dlink + `','`+pwd+`','`+ file.path +`')">`+file.server_filename+`</a>
<span class="float-right">`+ file.size +`</span>
	  </li>`
  ...
  if(json2 != 1){
    ...
    const uk = json2.uk 
    const filejson = await getFileList(shareid,uk,randsk,surl_1,pwd)
...
const playerpages=`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"/>
...
<style>
  body {
    background-image: url("https://i.loli.net/2020/08/14/UgIcbQDGSyhfpqs.png");
  }
...
<li class="nav-item">
<a class="nav-link" href="./help" target="_blank">下载帮助</a>
</li>
</ul>
</div>
</div>
...
<h5 class="alert-heading">在线播放</h5>
<hr>
<p class="card-text"><div id="dplayer"></div></p>
</div>
</div>
</div>
</div>
...
const player = async request => {
  const form2 = await request.formData()
  const player = form2.get('player')
  return new Response(playerpages+player.replace('778750','250528')+playerpages2, { headers: {'Content-Type': 'text/html;charset=UTF-8'} })
}
...
const download = async request => {
  ...
  var realLink = randsk.substring(7)
  
  realLink=realLink.replace('clienttype=25','clienttype=25&origin=dlna&vip=2&')
  realLink=realLink.replace('250528','778750')
...
<hr>
<p class="card-text"><a href="http://`+realLink+`" onclick= target=_blank>下载链接(http)</a> <a href="https://`+realLink+`" target=_blank>下载链接(https)</a><br><br><a>推送到Aria2(即将支持)</a><br><br><a href="./help">下载链接使用方法（必读）</a>
<form target='_blank' id="_playerPost" method="post" action="player">
  <input type="hidden" name="player" value="https://`+realLink+`" /> 
  <button onclick="document.getElementById('_playerPost').submit();" class="mt-4 mb-3 form-control btn btn-success btn-block">在线播放（支持视频和歌曲）</button>
</form>
</p>
</div>`
